 ...
 *
 ******************************************************************************/
 void I2C_MasterReadEepomData(uint8_t write_addr, uint8_t u8Addr,uint8_t *pu8Data,uint32_t u32Len)
{
	...
	while(1)
	{
		...
		switch(u8State)
		{
			case 0x08:   //发送完START信号
				...
				I2C_Send7bitAddress(write_addr, 0X00);   //发送从机地址
				break;
...
void I2C_MasterWriteEepromData(uint8_t write_addr, uint8_t u8Addr,uint8_t *pu8Data,uint32_t u32Len)
{
    uint8_t u8i=0,u8State;
	I2C_GenerateSTART(ENABLE);
	while(1)
	{
		...
		switch(u8State)
		{
			case 0x08:   //发送完START信号
				I2C_GenerateSTART(DISABLE);
				I2C_Send7bitAddress(write_addr,0X00);  //从设备地址发送
				break;
